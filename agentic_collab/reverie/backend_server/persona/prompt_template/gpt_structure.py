...
from utils import openai_api_key, use_openai, api_model, debug
...
#config_path = Path("../../../openai_config.json")
#config_path = Path("/Users/josephkambeitz/Desktop/Research/simulacra/agentic_collab/openai_config.json")
...
def ChatGPT_request(prompt):
  ...
  try: 
    ...
    # Log to SimulationManager if available
    _log_api_call_to_simulation_manager(prompt, content, completion, "general")
...
def ChatGPT_structured_request(prompt, response_format):
  ...
  # Joseph commented this out in order to avoid cluttered console output
  #print("Prompt:", prompt, flush=True)
  ...
  try: 
    ...
    # Log to SimulationManager if available
    response_content = str(message.parsed) if message.parsed else str(message.refusal)
    _log_api_call_to_simulation_manager(prompt, response_content, completion, "structured")
...
def ChatGPT_safe_generate_structured_response(
  ...
  if func_validate and func_clean_up:
    ...
    if verbose:
      ...
      # Joseph commented these 3 lines out in order to avoid a cluttered console output
...
def _log_api_call_to_simulation_manager(prompt, response, completion, prompt_type):
    """
    Log API call to SimulationManager centralized logs if available
    """
    try:
        # Try to import and access SimulationManager
        import sys
        import os
        sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
        from simulation_manager import simulation_manager
        
        # Try to get current simulation context (this is a bit hacky but works)
        # Look for simulation context in various places
        sim_name = None
        agent_name = "unknown"
        step = 0
        ...
        try:
            # Look up the call stack for simulation context
            for i in range(10):  # Check up to 10 frames up
                frame = frame.f_back
                if frame is None:
                    break
                
                # Look for simulation context in local variables
                if 'self' in frame.f_locals:
                    obj = frame.f_locals['self']
                    if hasattr(obj, 'sim_code'):
                        sim_name = obj.sim_code
                    if hasattr(obj, 'step'):
                        step = obj.step
                    if hasattr(obj, 'scratch') and hasattr(obj.scratch, 'name'):
                        agent_name = obj.scratch.name
                    elif hasattr(obj, 'name'):
                        agent_name = obj.name
                
                # Also check for direct variables
                if 'sim_code' in frame.f_locals:
                    sim_name = frame.f_locals['sim_code']
                if 'agent_name' in frame.f_locals:
                    agent_name = frame.f_locals['agent_name']
                if 'step' in frame.f_locals:
                    step = frame.f_locals['step']
        finally:
            del frame
        
        # If we found a simulation name, log the API call
        if sim_name:
            ...
            if hasattr(completion, 'usage'):
                ...
                input_cost = openai_config.get("model-costs", {}).get("input", 0.0)
                output_cost = openai_config.get("model-costs", {}).get("output", 0.0)
                cost = (tokens["prompt_tokens"] * input_cost / 1000000 + 
                       tokens["completion_tokens"] * output_cost / 1000000)
            
            # Determine prompt type from content if not specified
            if prompt_type == "general":
                prompt_lower = prompt.lower()
                if "phq-9" in prompt_lower or "assessment" in prompt_lower:
                    prompt_type = "assessment"
                elif "plan" in prompt_lower or "schedule" in prompt_lower:
                    prompt_type = "planning"
                elif "conversation" in prompt_lower or "chat" in prompt_lower:
                    prompt_type = "conversation"
                elif "reflect" in prompt_lower or "insight" in prompt_lower:
            ...
            logger.log_api_call(
                ...
                response=response[:1000] + "..." if len(str(response)) > 1000 else str(response),  # Truncate long responses
                cost=cost,
                tokens=tokens
            )
            
    except ImportError:
        # SimulationManager not available, skip logging
        pass
    except Exception as e:
        # Don't break the API call if logging fails
        print(f"DEBUG: Failed to log API call to SimulationManager: {e}")
        pass
